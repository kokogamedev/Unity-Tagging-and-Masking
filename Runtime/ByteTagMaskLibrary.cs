...
{
	[CreateAssetMenu(fileName = "ByteTagMaskLibrary", menuName = "PsigenVision/ByteTagMaskLibrary")]

	public class ByteTagMaskLibrary : ScriptableObject
    {
        /// <summary>
	    /// Represents an array of fixed-size string labels corresponding to the 8 bits in an byte.
        ...
        /// </remarks>
	    [SerializeField] private string[] labels = new string[8]; // A fixed array of 8 strings (mapping to the bits in an byte)

        /// <summary>
        /// Gets the array of labels used for identifying and associating bit positions in a bitmask.
	    /// Each label corresponds to one of the 8 bits in an byte, enabling a mapping between human-readable
        ...
        /// <returns>
	    /// An int representing the bit value derived from the index of the label in the labels array.
        /// Returns -1 if the label is not found.
        /// </returns>
        public int ToIntMask(string label) // We only use this when we absolutely MUST convert a string to a bit.
        {
        ...
        /// <summary>
        /// Converts a label to its corresponding bit position in a bitmask or returns a default value if the label is not found.
        /// </summary>
        /// <param name="label">The label to look up in the labels array.</param>
        /// <param name="defaultValue">The default bitmask byte to return if the label is not found in the labels array.</param>
        /// <returns>
        /// A byte representing the bit value derived from the index of the label in the labels array.
        /// Returns 0 value if the label is not found.
        /// </returns>
        public byte ToMask(string label)
        {
            for (int i = 0; i < labels.Length; i++)
            {
                // Using Ordinal comparison is the fastest way to compare strings in C#
                if (string.Equals(labels[i], label, System.StringComparison.Ordinal))
                {
                    return (byte)(1 << i); // Return the bit value derived from the index (the bit position)
                }
            }
            return 0; // Not found
        }
        ...
        /// </returns>
	    public string ToLabel(byte bit) // We only use this when we absolutely MUST convert the bit to the appropriate label according to this TagMaskLibrary
        {
